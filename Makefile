...
SOC_DIR 	 := $(DEST_DIR)/soc
HEX_DIR 	 := $(DEST_DIR)/hex
...
$$($(1)_test_target): %: $$(SRC_DIR)/$(1)/%.S
	...
	$$(RISCV_PREFIX)-objcopy -O binary $$(IMG_DIR)/$(1)_$$@ $$(SOC_DIR)/$(1)_$$@
	od -v -An -tx1 $$(SOC_DIR)/$(1)_$$@ > $$(HEX_DIR)/$(1)_$$@
...
$(DEST_DIR) $(IMG_DIR) $(ELF_DIR) $(BIN_DIR):
	mkdir -p $(DEST_DIR) $(IMG_DIR) $(ELF_DIR) $(BIN_DIR) $(SOC_DIR) $(HEX_DIR)
...
all: $(DEST_DIR) $(IMG_DIR) $(ELF_DIR) $(BIN_DIR) $(SOC_DIR) $(HEX_DIR) $(tests)
